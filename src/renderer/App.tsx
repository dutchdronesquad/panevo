...
const syncPresetEntriesFromOnvif = (
  camera: CameraProfile,
  result: OnvifProbeResult,
): CameraPreset[] => {
  const existingByNumber = new Map(
    camera.presets.map((preset) => [preset.cameraPreset, preset]),
  );
  ...
  return result.presets
    ...
    .map((preset, index) => {
      ...
      return {
        id:
          existingPreset?.id ??
          `preset-onvif-${camera.id}-${preset.token.replace(/[^a-z0-9-]/gi, "-")}-${Date.now()}-${index}`,
...
export const App = () => {
  ...
  useEffect(() => {
    const candidates = config.cameras.filter(
      (camera) => shouldAutoProbeOnvif(camera) && !onvifProbeStates[camera.id],
    );
    ...
    void (async () => {
      ...
      for (const camera of candidates) {
        ...
        if (presetsChanged(camera.presets, nextPresets)) {
          workingConfig = updateCameraProfile(workingConfig, {
            ...camera,
            presets: nextPresets,
          });
  ...
  const saveCameraProfile = useCallback(
    (camera: CameraProfile) => {
      const previousCamera = config.cameras.find(
  ...
  const actions = useMemo(
    () => ({
      ...
      moveDownLeft: () =>
        handleResult(window.panevo.moveDownLeft(speed, speed)),
      moveDownRight: () =>
        handleResult(window.panevo.moveDownRight(speed, speed)),
      ...
      recallPreset: (preset: number) =>
        handleResult(window.panevo.recallPreset(preset)),
      storePreset: (preset: number, label?: string) =>
        handleResult(window.panevo.storePreset(preset, label)),
      addPreset: () => {
        void (async () => {
          if (activeCamera.presets.length >= 9) return;
          const usedNumbers = new Set(
            activeCamera.presets.map((preset) => preset.cameraPreset),
          );
          let cameraPreset = 1;
          ...
          const storeResult = await window.panevo.storePreset(
          ...
          await saveConfigState(
            updateActiveCamera(config, {
              ...activeCamera,
              presets: [...activeCamera.presets, nextPreset],
            }),
          );
        })();
      },
          ...
          const nextPreset: CameraPreset = {
            ...
            cameraPreset:
              updates.cameraPreset !== undefined
                ? Math.min(255, Math.max(1, Math.round(updates.cameraPreset)))
                : currentPreset.cameraPreset,
          };

          if (
            nextPreset.label !== currentPreset.label ||
            nextPreset.cameraPreset !== currentPreset.cameraPreset
          ) {
            const storeResult = await window.panevo.storePreset(
              nextPreset.cameraPreset,
              nextPreset.label,
        ...
        void (async () => {
          ...
          if (
            ...
            await saveConfigState(
              updateActiveCamera(config, {
                ...activeCamera,
                presets: activeCamera.presets.filter((item) => item.id !== id),
              }),
            );
            return;
          }

          const removeResult = await window.panevo.removePreset(
            preset.cameraPreset,
          );
          if (!removeResult.ok) {
            setError(
