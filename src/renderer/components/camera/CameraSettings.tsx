...
export const CameraSettings = ({ camera, onCameraChange, onSave, onTestConnection }: CameraSettingsProps) => {
  const usesOnvifSync = camera.syncProtocol === 'onvif' || camera.controlProtocol === 'onvif';
    ...
    <div className="card camera-settings-card">
      ...
      <div className="form-stack">
        ...
        <div className="field-row">
          ...
          {usesOnvifSync && (
            <div className="field">
              <Label htmlFor="onvif-port" className="field-label">ONVIF port</Label>
              <Input
                id="onvif-port"
                type="number"
                min={1}
                ...
                onChange={(e) => onCameraChange({ ...camera, onvifPort: Number(e.target.value) })}
              />
            </div>
          )}
        </div>
        ...
        <div className="field-row">
          <div className="field">
            <Label htmlFor="control-protocol" className="field-label">Live control</Label>
            <Select
              value={camera.controlProtocol}
              onValueChange={(v) => onCameraChange({ ...camera, controlProtocol: v === 'onvif' ? 'onvif' : 'visca' })}
            >
          ...
          {camera.controlProtocol === 'visca' && (
            <div className="field">
              <Label htmlFor="camera-protocol" className="field-label">Transport</Label>
              <Select
          ...
          <div className="field">
            <Label htmlFor="sync-protocol" className="field-label">Sync</Label>
            <Select
              value={camera.syncProtocol}
              onValueChange={(v) => onCameraChange({ ...camera, syncProtocol: v === 'none' ? 'none' : 'onvif' })}
            >
        ...
        {usesOnvifSync && (
          <div className="field-row">
            <div className="field">
              <Label htmlFor="onvif-username" className="field-label">ONVIF username</Label>
              <Input
                id="onvif-username"
                value={camera.onvifUsername}
                placeholder="Optional"
                autoComplete="username"
                onChange={(e) => onCameraChange({ ...camera, onvifUsername: e.target.value })}
              />
            </div>

            <div className="field">
              <Label htmlFor="onvif-password" className="field-label">ONVIF password</Label>
              <Input
                id="onvif-password"
                type="password"
                ...
                onChange={(e) => onCameraChange({ ...camera, onvifPassword: e.target.value })}
              />
            </div>
          </div>
        )}
