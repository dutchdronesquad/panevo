import { AlertTriangle, Camera, CheckCircle2, CircleOff, Gamepad2, RadioReceiver, Settings } from 'lucide-react';
...
export const AppSidebar = ({ activeView, activeCamera, status, error, onViewChange }: AppSidebarProps) => {
  const isError = !!error;
  const isVerified = status.connected && (status as { responseVerified?: boolean }).responseVerified === true && !isError;
  const isTransportReady = status.connected && (status as { responseVerified?: boolean }).responseVerified !== true && !isError;

  const chipClass = isError ? 'chip-error' : isVerified ? 'chip-live' : isTransportReady ? 'chip-standby' : 'chip-info';
  const chipLabel = isError ? 'Error' : isVerified ? 'Verified' : isTransportReady ? 'Transport' : 'Disconnected';
